...
public class NoiseOctavesR extends NoiseGenerator{
	
	/**
     * Collection of noise generation functions.  Output is combined to produce different octaves of noise.
     */
    private NoisePerlinR[] generatorCollection;
    private int octaves;

    public NoiseOctavesR(Random par1Random, int par2)
    {
        this.octaves = par2;
        this.generatorCollection = new NoisePerlinR[par2];

        for (int j = 0; j < par2; ++j)
        {
            this.generatorCollection[j] = new NoisePerlinR(par1Random);
        }
    }
