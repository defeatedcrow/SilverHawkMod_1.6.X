package mods.defeatedcrow.world.helper;
...
public class TeleporterReverseWorld extends Teleporter
{
    ...
    private static final int portalBlockID = SilverHawkCore.SHPortal.blockID;
    private static final int frameID = SilverHawkCore.DarkGlass.blockID;
    ...
    public TeleporterReverseWorld(WorldServer par1WorldServer)
    {
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        int j = 0;
        int k = 0;
        int i1 = MathHelper.floor_double(par1Entity.posX);//X座標
        int l1 = MathHelper.floor_double(par1Entity.posZ);//Z座標。変数の名前気になるから直した
        long j1 = ChunkCoordIntPair.chunkXZ2Int(i1, l1);//チャンク
        boolean flag = true;
        double d4;
        int k1;

        //portalの配置場所の検索に成功したら返ってくるフラグらしい
        ...
        {
            for (k1 = i1 - short1; k1 <= i1 + short1; ++k1)//X
            {
                double d5 = (double)k1 + 0.5D - par1Entity.posX;

                for (int l2 = l1 - short1; l2 <= l1 + short1; ++l2)
                {
                    double d6 = (double)l2 + 0.5D - par1Entity.posZ;//Z

                    //上（128）から順にY座標を走査。
                    for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2)
                    {
                        if (this.worldServerInstance.getBlockId(k1, i2, l2) == portalBlockID)//portal発見
                        {
                            while (this.worldServerInstance.getBlockId(k1, i2 - 1, l2) == portalBlockID)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlockId(i - 1, j, k) == portalBlockID)
            {
                j2 = 2;
            }
            ...
            if (this.worldServerInstance.getBlockId(i, j, k - 1) == portalBlockID)
            {
                j2 = 3;
            }
        ...
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? frameID : portalBlockID, 0, 2);
                }
            }
